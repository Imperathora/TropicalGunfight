...
{
	SteamCoreWeb = 0 UMETA(Hidden),Apps,Broadcast,CheatReporting,Community,Econ,EconMarket,Economy,GameInventory,GameNotifications,GameServers,
	GameServerStats,Inventory,LobbyMatchmaking,Leaderboards,MicroTxn,News,PlayerService,PublishedFile,PublishedItemSearch,PublishedItemVoting,
	RemoteStorage,User,UserAuth,UserStats,Workshop
};
...
public:
	...
	{
		if (key.Len() == 0 || data.Num() == 0) return;
		bool bHasEntry = Str.Len() > 0;
...
public:
	FString Get() 
	{
		if (Obj && !bFormatted)
		{
			auto m_JsonWriter = TJsonWriterFactory<TCHAR, TCondensedJsonPrintPolicy<TCHAR>>::Create(&JsonStr);
			FJsonSerializer::Serialize(Obj.ToSharedRef(), m_JsonWriter);
