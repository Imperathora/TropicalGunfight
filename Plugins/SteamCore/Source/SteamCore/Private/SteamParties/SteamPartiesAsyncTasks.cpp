...
void FOnlineAsyncTaskSteamCorePartiesCreateBeacon::Tick()
{
	...
	if (SteamUtilsPtr)
	{
		if (!bInit)
		{
			...
			FCStringAnsi::Strncpy(ConnectString, TCHAR_TO_UTF8(*m_ConnectString), m_ConnectString.GetAllocatedSize());
			char MetaData[512];
			FCStringAnsi::Strncpy(MetaData, TCHAR_TO_UTF8(*m_Metadata), m_Metadata.GetAllocatedSize());
