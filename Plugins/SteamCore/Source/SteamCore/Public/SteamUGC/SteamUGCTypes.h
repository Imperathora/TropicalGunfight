...
{
	Items = 0, Items_Mtx = 1, Items_ReadyToUse = 2, Collections = 3, Artwork = 4, Videos = 5, Screenshots = 6,
	AllGuides = 7, WebGuides = 8, IntegratedGuides = 9, UsableInGame = 10, ControllerBindings = 11, GameManagedItems = 12, All = 13,
};
...
{
	Published = 0, VotedOn, VotedUp, VotedDown, WillVoteLater, Favorited, Subscribed, UsedOrPlayed, Followed,
};
...
{
	CreationOrderDesc = 0, CreationOrderAsc, TitleAsc, LastUpdatedDesc, SubscriptionDateDesc, VoteScoreDesc, ForModeration,
};
...
{
	RankedByVote = 0, RankedByPublicationDate = 1, AcceptedForGameRankedByAcceptanceDate = 2, RankedByTrend = 3, FavoritedByFriendsRankedByPublicationDate = 4,	CreatedByFriendsRankedByPublicationDate = 5,
	RankedByNumTimesReported = 6, CreatedByFollowedUsersRankedByPublicationDate = 7, NotYetRated = 8, RankedByTotalVotesAsc = 9, RankedByVotesUp = 10, RankedByTextSearch = 11,
	RankedByTotalUniqueSubscriptions = 12, RankedByPlaytimeTrend = 13, RankedByTotalPlaytime = 14, RankedByAveragePlaytimeTrend = 15, RankedByLifetimeAveragePlaytime = 16,
	RankedByPlaytimeSessionsTrend = 17, RankedByLifetimePlaytimeSessions = 18,
};
...
{
	NumSubscriptions = 0, NumFavorites = 1, NumFollowers = 2, NumUniqueSubscriptions = 3, NumUniqueFavorites = 4, NumUniqueFollowers = 5,
	NumUniqueWebsiteViews = 6, ReportScore = 7, NumSecondsPlayed = 8, NumPlaytimeSessions = 9, NumComments = 10,
	NumSecondsPlayedDuringTimePeriod = 11, NumPlaytimeSessionsDuringTimePeriod = 12,
};
...
{
	First = 0, Community = 0, Microtransaction = 1, Collection = 2, Art = 3, Video = 4, Screenshot = 5,	
	Game = 6, Software = 7, Concept = 8, WebGuide = 9,IntegratedGuide = 10, Merch = 11, ControllerBinding = 12,	
	SteamworksAccessInvite = 13, SteamVideo = 14, GameManagedItem = 15,	Max = 16
...
public:
	...
	FCreateItemResult(const CreateItemResult_t& data)
		: Result(_SteamResult(data.m_eResult)), PublishedFileID(data.m_nPublishedFileId), bUserNeedsToAcceptWorkshopLegalAgreement(data.m_bUserNeedsToAcceptWorkshopLegalAgreement)
	{}
};
...
public:
	...
	FUserFavoriteItemsListChanged(const UserFavoriteItemsListChanged_t& data)
		: PublishedFileID(data.m_nPublishedFileId), Result(_SteamResult(data.m_eResult)), bWasAddRequest(data.m_bWasAddRequest)
	{}
};
...
public:
	...
	FGetUserItemVoteResult(const GetUserItemVoteResult_t& data)
		: PublishedFileID(data.m_nPublishedFileId), Result(_SteamResult(data.m_eResult)), bVotedUp(data.m_bVotedUp),
		bVotedDown(data.m_bVotedDown), bVoteSkipped(data.m_bVoteSkipped)
	{}
};
...
public:
	...
	FSetUserItemVoteResult(const SetUserItemVoteResult_t& data)
		: PublishedFileID(data.m_nPublishedFileId), Result(_SteamResult(data.m_eResult)), bVoteUp(data.m_bVoteUp)
	{}
};
...
public:
	...
	FSubmitItemUpdateResult(const SubmitItemUpdateResult_t& data)
		: Result(_SteamResult(data.m_eResult)), bUserNeedsToAcceptWorkshopLegalAgreement(data.m_bUserNeedsToAcceptWorkshopLegalAgreement), PublishedFileID(data.m_nPublishedFileId)
	{}
};
...
public:
	...
	FItemInstalled(const ItemInstalled_t& data)
		: AppID(data.m_unAppID), PublishedFileID(data.m_nPublishedFileId)
	{}
};
...
public:
	...
	FDownloadItemResult(const DownloadItemResult_t& data)
		: AppID(data.m_unAppID), PublishedFileID(data.m_nPublishedFileId), Result(_SteamResult(data.m_eResult))
	{}
};
...
public:
	...
	FSteamUGCQueryCompleted(const SteamUGCQueryCompleted_t& data)
		: Handle(data.m_handle), Result(_SteamResult(data.m_eResult)), NumResultsReturned(data.m_unNumResultsReturned), TotalMatchingResults(data.m_unTotalMatchingResults), bCachedData(data.m_bCachedData)
	{}
};
...
public:
	...
	FAddAppDependencyResult(const AddAppDependencyResult_t& data)
		: Result(_SteamResult(data.m_eResult)), PublishedFileID(data.m_nPublishedFileId), AppID(data.m_nAppID)
	{}
};
...
public:
	...
	FAddUGCDependencyResult(const AddUGCDependencyResult_t& data)
		: Result(_SteamResult(data.m_eResult)), PublishedFileId(data.m_nPublishedFileId), ChildPublishedFileId(data.m_nChildPublishedFileId)
	{}
};
...
public:
	...
	FGetAppDependenciesResult(const GetAppDependenciesResult_t& data)
		: Result(_SteamResult(data.m_eResult)), PublishedFileID(data.m_nPublishedFileId), NumAppDependencies(data.m_nNumAppDependencies), TotalNumAppDependencies(data.m_nTotalNumAppDependencies)
	{
