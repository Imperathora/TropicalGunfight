...
{
	Global = 0, GlobalAroundUser = 1, Friends = 2, Users = 3
};
...
{
	None = 0, Numeric = 1, TimeSeconds = 2, TimeMilliSeconds = 3
};

UENUM(BlueprintType)
enum class ESteamLeaderboardUploadScoreMethod : uint8
{
	None = 0, KeepBest = 1, ForceUpdate = 2
};
...
public:
	...
	FUserAchievementIconFetched(const UserAchievementIconFetched_t& data)
		: GameID(data.m_nGameID), AchievementName(UTF8_TO_TCHAR(data.m_rgchAchievementName)), bAchieved(data.m_bAchieved), m_nIconHandle(data.m_nIconHandle)
	{}
};
