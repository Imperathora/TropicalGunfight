...
{
	Init = 0,RequestLobbyList,RequestLobbyData,WaitForRequestLobbyData,Finished
};

UENUM(BlueprintType)
enum class ESteamLobbyType : uint8
{
	Private = 0, FriendsOnly = 1, Public = 2, Invisible = 3,
};
...
{
	EqualToOrLessThan = 0, LessThan = 1, Equal = 2, GreaterThan = 3, EqualToOrGreaterThan = 4, NotEqual = 5
};
...
{
	Close = 0, Default = 1, Far = 2, Worldwide = 3
};
...
public:
	...
	{
		if ((data.m_nFlags & (uint32)ESteamFavoriteFlags::None) > 0)
			Flags.Add(ESteamFavoriteFlags::None);
		if ((data.m_nFlags & (uint32)ESteamFavoriteFlags::Favorite) > 0)
			Flags.Add(ESteamFavoriteFlags::Favorite);
		if ((data.m_nFlags & (uint32)ESteamFavoriteFlags::History) > 0)
...
public:
		...
		: SteamIDLobby(data.m_ulSteamIDLobby), SteamIDUserChanged(data.m_ulSteamIDUserChanged), SteamIDMakingChange(data.m_ulSteamIDMakingChange)
	{
		...
		{
			if ((data.m_rgfChatMemberStateChange & static_cast<int32>(ESteamChatMemberStateChange(i))) > 0)
			{
