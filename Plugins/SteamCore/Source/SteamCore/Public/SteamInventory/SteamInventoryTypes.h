...
struct FSteamItemInstanceID
{
	GENERATED_BODY()
public:
	FSteamItemInstanceID() : Value(k_SteamItemInstanceIDInvalid){}
	FSteamItemInstanceID(uint64 value) : Value(value) {}
public:
...
struct FSteamItemDef
{
	GENERATED_BODY()
public:
	FSteamItemDef() : Value(0) {}
	FSteamItemDef(int32 data) : Value(data) {}
public:
...
public:
		...
		: InstanceID(data.m_itemId), Definition(data.m_iDefinition), Quantity(data.m_unQuantity)
	{
		for (int32 i = 0; i < 31; i++)
		{
			...
			{
				Flags.Add((ESteamCoreItemFlags)i);
			}
		}
	}
};
